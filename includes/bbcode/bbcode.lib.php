...
class bbcode {
    ...
    */
    ...
    function code_2html($elem) {
        ...
        $str .= '<strong>Код '.mb_strtoupper(htmlspecialchars($lang)).':</strong><br/>';
    ...
    function video_2html($elem) {
        ...
        foreach ($elem['val'] as $item) {
            ...
            $iframe_regexp      = '/<iframe.*?src=(?!"http:\/\/www\.youtube\.com\/embed\/|"http:\/\/vk\.com\/video_ext\.php\?|"'.$my_domen_regexp.').*?><\/iframe>/i';
    ...
    function audio_2html($elem) {
        ...
        $str .= '<object type="application/x-shockwave-flash" data="/includes/bbcode/player_mp3_mini.swf" width="200" height="20">
                     ...
                     <param name="FlashVars" value="mp3='.htmlspecialchars($elem['val'][0]['str']).'"></param>
                </object>';
        $str .= '</div>';
        return $str;
    }
    ...
    function img_2html($elem) {
		...
		$src = preg_replace ('/[^a-zA-Z0-9\-_\.\/\:]/ui', '', $src);
		$src = htmlspecialchars(str_replace ('..', '.', $src));
		...
		if (!mb_strstr($src, 'http://')){
			...
			if(file_exists(PATH.$src)){
				...
				if (!$zoom){
					...
					$html = '<div class="forum_zoom" style="width:'.$width.'px">'."\n";
						$html .= '<div><a href="'.$src.'" target="_blank"><img src="'.$src.'" '.htmlspecialchars($attr).' width="'.$width.'" height="'.$height.'" border="0"/></a></div>'."\n";
			...
			return '<div class="bb_img" style="'.$div_style.'"><img src="'.$src.'" '.htmlspecialchars($attr).' /></div>';
		}
    }
