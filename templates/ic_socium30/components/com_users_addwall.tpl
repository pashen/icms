{if $my_id}
    <form action="/users/wall-add" method="POST">
        ...
        <div style="text-align:right">
            <input class="button" type="submit" value="{$LANG.SEND}" />
            <input class="button" name="Button" type="button" value="{$LANG.CANCEL}" onclick="{literal}$('#addwall').slideToggle();$('.usr_wall_addlink').toggle();{/literal}"/>
        </div>
    </form>
{else}
