...
<form action="/catalog/{$id}/search.html" name="searchform" method="post" >
	...
	<p>
		<input class="button" type="submit" name="gosearch" value="{$LANG.SEARCH_IN_CAT}" />
		<input class="button" type="button" onclick="window.history.go(-1);" name="cancel" value="{$LANG.CANCEL}" />
	</p>
</form>