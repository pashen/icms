		...
		{else}
			{if $user_can_add}
				...
				{if $can_by_karma || !$cfg.min_karma}
					...
					<div class="cm_codebar" style="margin-top:5px;margin-bottom:15px">
						<table width="100%" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td width="" align="right">
									<input class="cm_submit button" type="submit" value="{$LANG.SEND}"/>
                                    <input class="cm_submit button" type="button" onclick="cancelComment({if $do=='add'}{$parent_id}{else}{$comment.id}{/if})" value="{$LANG.CANCEL}"/>
								</td>
							</tr>
						</table>
					</div>
