...
{if $is_blogs}
    ...
    <div class="blog_type_menu">
        ...
        {/if}
         ...
         {if $ownertype == 'all'}
            <span class="button blog_type_active">{$LANG.ALL_BLOGS} ({$total_blogs})</span>
         {else}
		...
		{if $single_blogs && $multi_blogs}
			{if $ownertype == 'single'}
				<span class="button blog_type_active">{$LANG.PERSONALS} <span class="blog_type_num">({$single_blogs})</span></span>
			{else}
		...
		{if $single_blogs && $multi_blogs}
			{if $ownertype == 'multi' && $multi_blogs}
				<span class="button blog_type_active">{$LANG.COLLECTIVES} <span class="blog_type_num">({$multi_blogs})</span></span>
			{else}
