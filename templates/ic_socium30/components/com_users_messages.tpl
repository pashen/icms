	<div class="con_heading" style="margin-bottom:25px">{$LANG.MY_MESS}</div>
		<div class="usr_msgmenu_tabs">
			{if $opt == 'in'}				
				<p class="usr_msgmenu_active in_span"><span><strong>{$LANG.INBOX}</strong></span></p> 
				<a class="usr_msgmenu_link out_link" href="/users/{$id}/messages-sent.html"><span><strong>{$LANG.SENT}</strong></span></a>
				<a class="usr_msgmenu_link new_link" href="/users/{$id}/messages-new.html"><span><strong>{$LANG.WRITE}</strong></span></a>
			{elseif $opt == 'out'}
				<a class="usr_msgmenu_link in_link" href="/users/{$id}/messages.html"><span><strong>{$LANG.INBOX}</strong></span></a> 
				<p class="usr_msgmenu_active out_span"><span><strong>{$LANG.SENT}</strong></span></p>
				<a class="usr_msgmenu_link new_link" href="/users/{$id}/messages-new.html"><span><strong>{$LANG.WRITE}</strong></span></a>
			{elseif $opt == 'new'}
				<a class="usr_msgmenu_link in_link" href="/users/{$id}/messages.html"><span><strong>{$LANG.INBOX}</strong></span></a> 
				<a class="usr_msgmenu_link out_link" href="/users/{$id}/messages-sent.html"><span><strong>{$LANG.SENT}</strong></span></a>
				<p class="usr_msgmenu_active new_span"><span><strong>{$LANG.WRITE}</strong></span></p>
			{elseif $opt == 'history'}
				<a class="usr_msgmenu_link in_link" href="/users/{$id}/messages.html"><span><strong>{$LANG.INBOX}</strong></span></a> 
				<a class="usr_msgmenu_link out_link" href="/users/{$id}/messages-sent.html"><span><strong>{$LANG.SENT}</strong></span></a>
				<a class="usr_msgmenu_link new_link" href="/users/{$id}/messages-new.html"><span><strong>{$LANG.WRITE}</strong></span></a>
				<p class="usr_msgmenu_active history_span"><span><strong>{$with_name}</strong></span></p>
			{/if}
		</div>
		...
		{if $opt == 'in' || $opt == 'out' || $opt == 'history'}
			...
			{if $is_mes}
                    ...
                    <div class="usr_msg_entry">
						...
						<tr>						
							<td width="70" height="70" valign="top" align="center">
							{$record.user_img}
							{if $record.is_online}
							...
							<td width="" valign="top">
								<div class="msg_text" style="padding:1px 20px 1px 1px">{$record.message}</div>
							</td>
							<td width="100" valign="top">
								<div class="msg_options">
									{if $opt=='in'}
									{if $record.sender_id>0}
										<div class="usr_msg_title"><a class="msg_reply" href="/users/{$record.from_id}/reply{$record.id}.html">{$LANG.REPLY}</a></div>
										<div class="usr_msg_title"><a class="msg_history" href="/users/{$id}/messages-history{$record.from_id}.html">{$LANG.HISTORY}</a></div>
									{/if}
									{/if}
									{if $opt=='in' || ($opt=='out' && !$record.is_new)}
										<div class="usr_msg_title"><a class="msg_delete" href="/users/delmsg{$record.id}.html">{$LANG.DELETE}</a></div>
									{/if}
								</div>
							</td>
						</tr>
		...
		{if $opt == 'new'}
			<form action="" id="newmessage" method="POST" name="msgform">			
				...
				<div>
						...
						<div style="margin-top:6px;"><input class="button" type="button" id="gosend" value="{$LANG.SEND}" onclick="sendMessage()" style="font-size:16px"/></div>
				</div>			
			</form>				
		{/if}
